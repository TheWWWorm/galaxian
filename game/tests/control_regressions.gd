...
func check_touch(app):
	...
	check(controls.touch_look.x > 0, "Empty-space drag steers")
	await touch(center, 4, true); await drag(center - Vector2(40, 0), 4)
	check(controls.touch_look.x > 0 and hud.stick_finger < 0, "Stick cannot overwrite existing empty-space drag")
	await touch(center, 4, false); await touch(empty, 3, false, true)
